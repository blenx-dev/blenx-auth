---
title: Beanie example
description: Wire up blenx-auth Beanie (MongoDB) repositories behind the same core services.
---

Beanie (MongoDB) example — same auth services on MongoDB. Runs with the `beanie` extra
installed and a running MongoDB:
...
This wires the Beanie repositories (instead of the SQLAlchemy ones) behind the same
services and settings, demonstrating that the core is storage-agnostic.
